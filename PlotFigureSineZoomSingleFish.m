...
for br = 1:size(BrainRegions, 2)
    disp(['Brain Region: ', num2str(br), '/', num2str(size(BrainRegions, 2))]);
...
%     end
    ...
    T = text((x+(x+ScaleBar/RatioPixMicron))/2, y+15, [num2str(ScaleBar), ' μm']);
...
%% Save Figures
% Local
saveas(F2, [FigureOutPath, '/', 'BinaryValueVest', '.fig']);
saveas(F2, [FigureOutPath, '/',  'BinaryValueVest', '.svg']);
% % Nextcloud
