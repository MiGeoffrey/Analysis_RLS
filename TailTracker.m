%%
path = '/home/ljp/Science/Projects/RLS/Data/2019-12-16/Run 02/17h17m00s-tailcroped.avi';
...
imshow(frame);
h = impoint;
position = wait(h);  x = position(1); y = position(2);
...
h = imline;
position = wait(h);
taillength = abs(position(3)-position(4));
...
mov = VideoReader(path);
frames = zeros(mov.Height, mov.Width, mov.NumFrames);
angle = zeros(mov.NumFrames);
count = 1; i_count = 1;
while hasFrame(mov)
    % Moment
    frame = readFrame(mov);
    ...
    count = count + 1;
    if i >= i_count
        clc; disp(['Frame number ', num2str(count), '/', num2str(mov.NumFrames)]);
        i_count = i_count +100;
    end
